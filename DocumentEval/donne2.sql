...
CREATE SEQUENCE "public".historiqueprix_idhistoriqueprix_seq START WITH 1 INCREMENT BY 1;

CREATE SEQUENCE "public".historiquetypefinition_idhistoriquetypefinition_seq START WITH 1 INCREMENT BY 1;
...
CREATE  TABLE "public".historiquetypefinition ( 
	idhistoriquetypefinition integer DEFAULT nextval('historiquetypefinition_idhistoriquetypefinition_seq'::regclass) NOT NULL  ,
...
CREATE  TABLE "public".historiqueprix ( 
	idhistoriqueprix     integer DEFAULT nextval('historiqueprix_idhistoriqueprix_seq'::regclass) NOT NULL  ,
...
ALTER TABLE "public".typefinition ADD CONSTRAINT fk_typefinition_devis FOREIGN KEY ( idtypefinition ) REFERENCES "public".devis( idtypefinition );
...
CREATE VIEW "public".montanttotalprixdejapayer AS SELECT sum(payement.montant) AS prixdejapayer
   FROM payement;
...
CREATE VIEW "public".devisavecresteapayer AS SELECT v_devis.iddevis,
    ...
    COALESCE(v_payement.resteapayer, (v_devis.totalprixdevis + ((v_devis.totalprixdevis * v_devis.pourcentagefinition) / (100)::double precision))) AS resteapayer,
    v_devis.idclient,
    v_devis.numero,
    ...
    COALESCE(((v_payement.resteapayer * (100)::double precision) / (v_devis.totalprixdevis + ((v_devis.totalprixdevis * v_devis.pourcentagefinition) / (100)::double precision))), (0)::double precision) AS pourcentagedejapayer
   FROM (v_devis
...
CREATE OR REPLACE FUNCTION public.effacer_donnees_sauf_utilisateur()
 RETURNS void
 LANGUAGE plpgsql
AS \$function\$
DECLARE
    r RECORD;
BEGIN
    -- D‚sactiver les d‚clencheurs sur toutes les tables de la base de donn‚es
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = current_schema()) LOOP
        EXECUTE 'ALTER TABLE ' || quote_ident(r.tablename) || ' DISABLE TRIGGER ALL';
    END LOOP;
    
    -- Supprimer toutes les donn‚es de toutes les tables, sauf celle de l'utilisateur
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = current_schema()) LOOP
    ...
    -- R‚activer les d‚clencheurs sur toutes les tables de la base de donn‚es
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = current_schema()) LOOP
